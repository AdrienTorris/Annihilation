...
namespace Engine.Mathematics
{
    public static class HalfUtility
    {
        ...
        private struct FloatToUint
        {
            ...
            public FloatToUint(uint uintValue)
            {
                FloatValue = 0f;
                UintValue = uintValue;
            }

            public FloatToUint(float floatValue)
            {
                UintValue = 0;
        ...
        {
            uint uintValue = HalfToFloatMantissaTable[HalfToFloatOffsetTable[h >> 10] + (((uint)h) & 0x3ff)] + HalfToFloatExponentTable[h >> 10];
            return new FloatToUint(uintValue).FloatValue;
        }

        public static ushort Pack(float f)
        {
            FloatToUint conv = new FloatToUint(f);
